...
int main(int argc, char *argv[]) {
  GstElement *pipeline, *source, *convert, *format, *sink;
  GstCaps *caps;
  GstBus *bus;
  GstMessage *msg;
  ...
  if (!pipeline || !source || !convert || !format || !sink) {
    g_printerr ("Not all elements could be created.\n");
    return -1;
  }

  /* Build the pipeline */
  gst_bin_add_many (GST_BIN (pipeline), source, convert, format, sink, NULL);
  if (!gst_element_link_many (source, convert, format, sink, NULL)) {
